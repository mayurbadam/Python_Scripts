...
def add_expense():
    ...
    # Read existing data to avoid duplications
    try:
        existing_sheet = pd.read_excel("excel_budget.xlsx", sheet_name=current_month, engine="openpyxl")
    except (FileNotFoundError, ValueError):
        existing_sheet = pd.DataFrame(columns=["Date", "Name", "Rent", "Need", "Transit", "Luxury", "Lent", "Savings"])
    ...
    # Write back to Excel, clearing the old data before writing new
...
def delete_expense():
    ...
    if selected_item:
        ...
        existing_sheet = existing_sheet[(existing_sheet.Date != date) | (existing_sheet.Name != name)]
        with pd.ExcelWriter("excel_budget.xlsx", engine="openpyxl", mode="w") as writer:
    ...
    if os.path.exists("expenses.txt"):
        with open("expenses.txt", "r") as file:
            for line in file:
                ...
                try:
                    total_expense += sum(int(v) if v.isdigit() else 0 for v in values[2:])
                except ValueError:
                    pass
